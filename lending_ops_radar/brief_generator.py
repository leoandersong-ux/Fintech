...
def row_link(row: sqlite3.Row) -> str:
...
def table_for(rows: Iterable[sqlite3.Row]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无已复核条目 | No reviewed items selected yet._"
    lines = [
        "| 信号 Signal | 分类 Classification | 风险 Risk | 来源 Source | 下一步 Next Action |",
        "| --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        action = readable_text(row["recommended_action"], fallback_next_action(row)) or fallback_next_action(row)
        lines.append(
            "| "
            + " | ".join(
                [
...
def findings_table(rows: Iterable[dict[str, str]]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无关键判断 | No interpretive findings yet._"
    lines = [
        "| 关键判断 Key Finding | 为什么重要 Why It Matters |",
        "| --- | --- |",
    ]
    for row in rows:
        finding = f"{row['finding_cn']} / {row['finding_en']}"
        why = f"{row['why_cn']} / {row['why_en']}"
    ...
    for index, row in enumerate(rows, start=1):
        ...
        lines.extend(
            [
                ...
                "",
                f"**信号 | Signal:** {readable_text(row['signal'])}",
                "",
                f"**对小微贷款业务的影响：** {readable_text(row['lending_impact_cn'])}",
                "",
                f"**Lending impact:** {readable_text(row['lending_impact_en'])}",
                "",
                f"**建议动作：** {readable_text(row['recommended_actions_cn'])}",
                "",
                f"**Recommended action:** {readable_text(row['recommended_actions_en'])}",
                "",
                f"**待验证问题：** {readable_text(row['follow_up_questions_cn'])}",
                "",
                f"**Follow-up question:** {readable_text(row['follow_up_questions_en'])}",
                "",
                f"**来源 | Source:** {link_for(row['source_link'])}",
                "",
            ]
        )
    return "\n".join(lines)
...
def coverage_gap_table(rows: Iterable[dict[str, str]]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无覆盖缺口判断 | No coverage-gap assessment yet._"
    lines = [
        "| 情报线 Intelligence Lane | 当前覆盖 Current Coverage | 主要缺口 Main Gap | 下一步来源 Next Sources |",
        "| --- | --- | --- | --- |",
    ]
    for row in rows:
        area = f"{row['area_cn']} / {row['area_en']}"
        coverage = f"{row['coverage_cn']} / {row['coverage_en']}"
        gap = f"{row['gap_cn']} / {row['gap_en']}"
        next_source = f"{row['next_source_cn']} / {row['next_source_en']}"
        lines.append(
...
def notes_table(rows: Iterable[sqlite3.Row]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无个人研究笔记 | No personal research notes yet._"
    lines = [
...
def questions_table(rows: Iterable[sqlite3.Row]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无市场问题 | No market questions seeded yet._"
    lines = [
...
def source_health_table(rows: Iterable[sqlite3.Row]) -> str:
    rows = list(rows)
    if not rows:
        return "_暂无来源健康记录 | No source health records yet._"
    lines = [
...
本周笔记包含 {len(rows)} 条已复核公开来源信号和 {len(notes)} 条个人研究笔记；其中 {high_impact_count} 条落在高潜在影响域，{medium_impact_count} 条落在中等潜在影响域，覆盖 {interpreted_domains} 个 lending ops 影响域。这里的“影响”是研究优先级，不是合规结论。
...
def main() -> int:
    ...
    content = render_brief(rows, week_label, notes, questions, source_health)
